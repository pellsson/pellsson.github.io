...
We set out to have a single run getting the #1 spot in all three "major-scores": highest score, lowest turn count, and fastest realtime on NAO. We’d need an RNG-predicting bot.
...
We went back on forth on several methods, but ultimately settled on building a gigantic database. Create a hash of the inventory and starting attributes for **all** `Tou Hum Fem Neu` games possible, and save them in a binary sorted lookup table.
